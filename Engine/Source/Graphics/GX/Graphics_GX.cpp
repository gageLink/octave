...
void GFX_BeginFrame()
{
    ...
    Mtx44 projection;
    Camera3D* camera = gGxContext.mWorld->GetActiveCamera(Renderer::Get()->GetCurrentScreen());
...
void GFX_DrawStaticMeshComp(StaticMesh3D* staticMeshComp, StaticMesh* meshOverride)
{
    ...
    if (mesh != nullptr)
    {
        ...
        glm::mat4 viewSrc = glm::transpose(gGxContext.mWorld->GetActiveCamera(Renderer::Get()->GetCurrentScreen())->GetViewMatrix());
